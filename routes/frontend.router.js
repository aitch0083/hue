...
var category_read_handler = function(req, res, next){
	...
	Promise.join(
		...
		if(!category || category === null || category.Dispaly != 1){
			var error = new Error('Invalid Category. ' + category.Dispaly);
			console.error('Invalid category caught:', id, ', display: ', category.Dispaly);
			next(error);
			return;
		}
		...
		Article.findAll({
			...
			_.each(articles, function(article){
				...
				xml.ele('article')
				   ...
				   .ele('updateTimeUnix', null, update_time.add('7', 'm').valueOf()).up()
